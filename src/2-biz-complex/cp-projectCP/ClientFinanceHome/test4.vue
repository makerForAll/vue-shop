<template>
    ...
    <div class="tablestyle-div">
        <a-table size="small" id="printMeReadClientFinanceHome" @expand="onExpandChange" :row-class-name="'tablestyle'"
            :columns="columns" :data-source="transformedData" class="components-table-demo-nested">
            ...
            <template #expandedRowRender="{ record }">
                <a-table size="small" :scroll="{ x: 800, y: 'auto' }" :row-class-name="'tablestyle2'"
                    ...
                    <template #expandedRowRender="{ record, index: outerIndex }">
                        <a-table :scroll="{ x: 1000, y: 500 }" size="small" bordered :columns="inner2Columns"
                            :data-source="record.payment" :pagination="false">
                            <template #bodyCell="{ column, text, index }">
                                <template v-if="column.key === 'index'">
                                    <span>
...
const innerColumns = [
    ...
    { title: '备注', dataIndex: 'remarks', key: 'remarks', width: '36%', align: innerColumnsAlign.value }
];
...
const inner2Columns = [
    ...
    { title: '状态', dataIndex: 'state', key: 'state', width: '10%', align: inner2ColumnsAlign.value },
    { title: '实际支付日期', dataIndex: 'paymentDate', key: 'paymentDate', width: '15%', align: inner2ColumnsAlign.value },
    { title: '备注', dataIndex: 'remarks', key: 'remarks', width: '36%', align: inner2ColumnsAlign.value },
    { title: '依据链接', dataIndex: 'link', key: 'link', width: '13%', className: 'innerc2-link', align: inner2ColumnsAlign.value }
...
function transformAPIData(apiData: any[]): DataItem[] {
    return apiData.map(plan => ({
        ...
        amount: `￥${plan.total_amount}`, // 将总金额格式化为金额字符串
