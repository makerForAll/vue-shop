      ...
      <a-dropdown>
        <template #overlay>
          <a-menu @click="handleMenuClick">
            <a-menu-item key="1" @click="generatePaymentDetails">
              生成 - 支付明细【精度：个位】</a-menu-item
            >
            <a-menu-item key="3" @click="generatePaymentDetails2"
              >生成 - 支付明细【精度：小数点后2位】</a-menu-item
            >
  ...
  () =>
    [
      ...
      },
      {
        name: 'rel_store',
        label: '不动产单元',
        component: 'a-select-obj',
        props: {
          // options_obj: options_objArr.value ? options_objArr.value : []
          options_obj: planStore.data.unityItems
        },
        span: 24
      },
      // {
      ...
      },
      {
        ...
        label: '增长率（百分比）',
        component: 'a-input-number',
        props: { placeholder: '6', addonAfter: '%' },
        span: 8
      },
      {
        name: 'deposit',
        label: '押金',
        component: 'a-input-number',
        props: { placeholder: '500', addonAfter: '¥' },
        span: 8
      },
      {
      ...
      {
        name: 'rent_free_months',
        label: 'JM',
        component: 'a-input-number',
        props: { placeholder: 'Enter rent free (months)', addonAfter: '月' },
        span: 8
      },
      {
        name: 'remarks',
...
const generatePaymentDetails = async () => {
  ...
  const {
    paymentDetails,
    paymentAmountArr
  }: { paymentDetails: CPaymentDetailItemVO[]; paymentAmountArr: number[] } = await paymentMainFun(
    planStore.data.CUItem as RPlanVO,
    '0'
  )
...
const generatePaymentDetails2 = async () => {
  // 生成支付明细[精度到小数点后2位]
  // 放入plan的基础数据，获得支付明细的数据
  const {
    paymentDetails,
