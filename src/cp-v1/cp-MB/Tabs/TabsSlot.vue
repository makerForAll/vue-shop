...
// ------------------ 实现懒加载，按需加载 ---------------------------------
const activeKey = ref('0'); // 默认激活的标签页
const loadedTabs = ref<string[]>(['0']); // 已加载内容的标签页
...
<template>
  <a-tabs v-model:activeKey="activeKey" type="card" @change="handleChange">
    ...
    <a-tab-pane key="4" tab="已归档">
      <div v-if="loadedTabs.includes('5')">
        <slot name="4-container">
          4-1
        </slot>
        <!-- 归档信息<br>
