<template>
  ...
  <a-drawer v-if="isDelete" :title="action + entityType" :width="820" :open="isOpen"
    :body-style="{ paddingBottom: '120px' }" :footer-style="{ textAlign: 'right' }" @close="closeDrawer"
              ...
              <a-form-item :label="field.labelNone ? '' : field.label" :name="`${field.name}`"
                ...
                <template v-else-if="field.component === 'a-select'">
                  <a-select :id="`form_item_${field.name}`" :name="`form_item_${field.name}`" @change="fieldChange"
                    ...
                    <a-select-option v-for="option in field.props?.options_str" :key="option.value"
                      :value="option.value">
                      {{ option.label }}
                    </a-select-option>
                  </a-select>
                ...
                <template v-else-if="field.component === 'a-select-number'">
                  <a-select :id="`form_item_${field.name}`" :name="`form_item_${field.name}`" @change="fieldChange"
                    v-model:value="internalForm[field.name]" v-bind="field.props">
                    <a-select-option v-for="option in field.props?.options_num" :key="option.value"
                      :value="option.value">
                      {{ option.label }}
                    </a-select-option>
                  </a-select>
                ...
                <template v-else-if="field.component === 'a-range-picker'">
                  <a-space direction="vertical" :size="12">
                    <a-range-picker :id="`form_item_${field.name}`" :name="`form_item_${field.name}`"
                      v-model:value="internalForm[field.name]" :presets="rangePresets" @calendarChange="onRangeChange"
                ...
                <template v-else-if="field.component === 'a-date-picker'">
                  <a-date-picker :id="`form_item_${field.name}`" :name="`form_item_${field.name}`" @change="fieldChange"
                    @open-change="fieldChange" v-model:value="internalForm[field.name]" v-bind="field.props" />
                ...
                <template v-else-if="field.component === 'a-input-group'">
                  ...
                  <a-row>
                    <template v-for="(item, index) in internalForm[field.name]" :key="`${field?.name}_${item[index]}`">
                      <a-col justify="center" :span="4" style="border:0.5px black dashed; text-align: center;">
                        <span>{{ index + 1 }}</span>
                      </a-col>
                      <!-- {{ item }} -->
                      <template v-for="k in field.group" :key="k">
                        ...
                        <a-col :span="4" style="border:0.5px black dashed">
                          <a-form-item style="height: auto;">
                            ...
                            <a-input-number v-if="k == 'amount'" style="width: 105px;" v-model:value="item[k]"
                              :id="`form_item_${field.name}_${k}_${index}`" @input="fieldChange" @change="fieldChange"
                              :name="`form_item_${field.name}_${k}_${index}`" />

                            <a-input v-else-if="k == 'remarks'" style="width: 105px;" v-model:value="item[k]"
                              :id="`form_item_${field.name}_${k}_${index}`" @input="fieldChange" @change="fieldChange"
                              :name="`form_item_${field.name}_${k}_${index}`" />

                            <a-date-picker v-else-if="k == 'period_start' || 'period_end' || 'due_date'"
                              v-model:value="item[k]" @change="fieldChange" @open-change="fieldChange" />

                            <a-input v-else style="width: 105px;" v-model:value="item[k]"
                              :id="`form_item_${field.name}_${k}_${index}`" @input="fieldChange" @change="fieldChange"
                              :name="`form_item_${field.name}_${k}_${index}`" />
                          </a-form-item>
                        </a-col>
...
watch(
  ...
  (newValue) => {
    ...
    console.log('internalForm watch--------------------------:', newValue.loading)
  },
  {
    deep: true,
    // immediate: true
