...
// import type { CSoftwareVO, USoftwareVO } from '@/0-manage-hub/services/vo/software'
// import softwareService from '@/0-manage-hub/services/softwareService'
import type { CPartySoftwareVO, UPartySoftwareVO } from '@/0-manage-hub/services/vo/partysoftware'
import partysoftwareService from '@/0-manage-hub/services/partysoftwareService'

export const usePartySoftwareStore = defineStore('partysoftware', {
  // 其他配置...
  state: () => {
    // 可以写一些逻辑
    return {
      data: {
        tabsKey: 0 as number,
        CItem: {} as CPartySoftwareVO,
        UItem: {} as UPartySoftwareVO,
        RItem: {} as UPartySoftwareVO,
        RItems: [] as UPartySoftwareVO[],
        total: 0,
        selectName: '',
        selectID: '', // 要更新的对象
        loading: false, // store 的数据 (data)
        partyItems: [] as any[],
        softwareItems: [] as any[]
      }
    }
  },
  getters: {
  ...
  actions: {
    ...
    async create(data: any) {
      ...
      const response = (await partysoftwareService.create(data)) as ApiResults<UPartySoftwareVO>
    ...
    async read(
      ...
      filters: any = {}
    ): Promise<UPartySoftwareVO[]> {
      this.data.loading = true
      // 异步编程获取数据
      const response = (await partysoftwareService.read(
        params,
        filters
      )) as ApiResults<UPartySoftwareVO>
      if (response) {
        this.data.RItems = response.data as UPartySoftwareVO[]
    ...
    async readById(id: string | undefined): Promise<UPartySoftwareVO> {
      this.data.loading = true
      const response = (await partysoftwareService.readById(id)) as ApiResults<UPartySoftwareVO>
    ...
    async readByIdForRel(id: string): Promise<UPartySoftwareVO> {
      this.data.loading = true
      const response = (await partysoftwareService.readByIdForRel(
        id
      )) as ApiResults<UPartySoftwareVO>
      if (response) {
        this.data.UItem = response.data[0]
    ...
    async update(id: string | undefined, requestBody: any) {
      // console.log('id---', id)
      this.data.loading = true
      const response = (await partysoftwareService.update(
        id,
        requestBody
      )) as ApiResults<UPartySoftwareVO>
      if (response) {
    ...
    async delete(id: string) {
      try {
        await customApi.softwareControllerDelete(id)
        // 更新本地 state 中的 party 数据 -------------------------------------------
        const index = this.data.RItems.findIndex((item: any) => item.id === id)
        if (index !== -1) {
