...
export const useSoftwareStore = defineStore('software', {
  ...
  state: () => {
    ...
    return {
      data: {
        tabsKey: 0 as number,
        CItem: {} as CSoftwareVO,
        UItem: {} as USoftwareVO,
        RItem: {} as USoftwareVO,
        RItems: [] as USoftwareVO[],
        total: 0,
        selectName: '',
        selectID: '', // 要更新的对象
  ...
  actions: {
    ...
    async create(data: any) {
      ...
      const response = (await softwareService.create(data)) as ApiResults<USoftwareVO>
    ...
    async read(
      ...
      this.data.loading = true
      // 异步编程获取数据
      const response = (await softwareService.read(params, filters)) as ApiResults<USoftwareVO>
      if (response) {
        this.data.RItems = response.data as USoftwareVO[]
    ...
    async readById(id: string): Promise<USoftwareVO> {
      this.data.loading = true
      const response = (await softwareService.readById(id)) as ApiResults<USoftwareVO>
      if (response) {
    ...
    async update(id: string, requestBody: any) {
      // console.log('id---', id)
      this.data.loading = true
      const response = (await softwareService.update(id, requestBody)) as ApiResults<USoftwareVO>
      if (response) {
        this.data.UItem = response.data[0]
        // 更新本地缓存
