<template>
    <DrawerSlotView :action="'更新'" :entityType="''" v-model="partyStore.data.UItem" :fields="clientFields"
        :rules="rules" :loading="partyStore.data.loading" @handleSubmit="handleSubmit" @showDrawer="showDrawer"
...
const clientFields = [
    ...
    { name: 'sorting', label: '排序', component: 'a-input', props: { placeholder: 'Enter phone number' } },
    // 其他字段可以按需添加
];
...
const showDrawer = async () => { // 读取 指定ID的 数据 
    console.log("ID------------", partyStore.data.selectID);
    await partyStore.readById(partyStore.data.selectID);
    //  为什么没有生效
}
...
const handleSubmit = async (form: any) => { // 更新指定数据 // Create / Update
    ...
    console.log("ID------------", partyStore.data.selectID, form);

    await partyStore.update(partyStore.data.selectID, form);
