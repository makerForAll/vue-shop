...
class ClientService {
  ...
  async readClientsByPartyId(
    ...
    try {
      ...
      const response = await customApi.clientControllerFindClients({ partyId, ...requestParams })
  ...
  async createClientByPartyId(partyId: string, form: any) {
    try {
      const response = await customApi.clientControllerCreateClientByParty(partyId, form)
      const getResponseData = response.data as unknown as ApiResponse<CClientVO>
      if (Array.isArray(getResponseData.results.data)) {
        const data = convertDatesToDayjs(getResponseData.results.data)
        return { data }
