...
<template>
  ...
  <a-table :data-orientation="commonStore.printOrientation" style="padding:5px;" size="small" :bordered="true"
    id="printMeReadClientHome" :scroll="{ x: 1200, y: 500 }" :columns="clientColumns"
...
const handleMouseOver = async (event: MouseEvent, id: string, name: string) => {
  ...
  await clientStore.updateSelectID(id, name);
...
const queryData = async (params: any, filters: any): Promise<void> => {
  ...
  if (params) {
    ...
    await clientStore.readClientsByPartyId(props.partyId, params);
  }
