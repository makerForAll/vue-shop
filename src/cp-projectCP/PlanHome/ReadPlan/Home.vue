<template>
  ...
  <a-drawer v-model:open="open" :title="`${clientStore.data.selectName}-方案列表`" width="820" :closable="false"
    ...
    <div>
      ...
      <a-skeleton :loading="planStore.data.loading">
        <a-tabs :tabBarStyle="{ backgroundColor: 'lightblue', padding: '16px' }" v-model:activeKey="activeKey" hide-add
          @change="(key: any) => tabClick(key)" type="editable-card" @edit="onEdit">
          <a-tab-pane :style="{
            transform: `scale(${headerStore.getScale})`,
            transformOrigin: 'top left',
          }" v-for="pane in planStore.data.RItems" :key="pane.id" :tab="pane.name" :closable="false"
            ...
            <div class="PrintMeReadPlan" style="border:1px solid gray; padding:20px;" :id="`printMe${pane.id}`">
              <a-skeleton :loading="planStore.data.loadingForTable">
                ...
                <label :for="`${pane.id}` + 'initial_monthly_price'">计租期开始约租金价格(元/每月):</label><input
                  :id="`${pane.id}` + 'initial_monthly_price'" :name="`${pane.id}` + 'initial_monthly_price'"
                  :disabled="true" :value="pane.initial_monthly_price">
                <br>
              </a-skeleton>
              <label :for="`${pane.id}` + 'total_area'">面积约(平方)：</label><input :id="`${pane.id}` + 'total_area'"
                :name="`${pane.id}` + 'total_area'" :disabled="true" :value="pane.total_area"><br>
              <h1>二、合同基础信息</h1>
              <label :for="`${pane.id}` + 'startdate_and_enddate[0]'">合同生效起始日期：</label><input
                :id="`${pane.id}` + 'startdate_and_enddate[0]'" :name="`${pane.id}` + 'startdate_and_enddate[0]'"
                :disabled="true"
                :value="dayjs(pane.startdate_and_enddate ? pane.startdate_and_enddate[0] : '').format('YYYY-MM-DD')">
              <br />
              <label :for="`${pane.id}` + 'startdate_and_enddate[1]'">合同生效截止日期：</label><input
                :id="`${pane.id}` + 'startdate_and_enddate[1]'" :name="`${pane.id}` + 'startdate_and_enddate[1]'"
                :disabled="true"
                :value="dayjs(pane.startdate_and_enddate ? pane.startdate_and_enddate[1] : '').format('YYYY-MM-DD')"><br />
              <label :for="`${pane.id}` + 'contract_duration_days'">合同天数：</label><input
                :id="`${pane.id}` + 'contract_duration_days'" :name="`${pane.id}` + 'contract_duration_days'"
                :disabled="true" :value="pane.contract_duration_days">
              <a-skeleton :loading="planStore.data.loadingForTable">
                <br />
                <label :for="`${pane.id}` + 'payment_interval_months'">租金缴纳周期(每多少月)</label><input
                  :id="`${pane.id}` + 'payment_interval_months'" :name="`${pane.id}` + 'payment_interval_months'"
                  :disabled="true" :value="pane.payment_interval_months"><br />
                <label :for="`${pane.id}` + 'increase_interval_months'">增长率条件(每多少月)：</label><input
                  :id="`${pane.id}` + 'increase_interval_months'" :name="`${pane.id}` + 'increase_interval_months'"
                  :disabled="true" :value="pane.increase_interval_months"><br>
                <label :for="`${pane.id}` + 'increase_rate'">增长率(百分之)：</label><input
                  :id="`${pane.id}` + 'increase_rate'" :name="`${pane.id}` + 'increase_rate'" :disabled="true"
                  :value="pane.increase_rate"><br>
                <label :for="`${pane.id}` + 'deposit'">押金(元)：</label><input :id="`${pane.id}` + 'deposit'"
                  :name="`${pane.id}` + 'deposit'" :disabled="true" :value="pane.deposit"><br>
                <label :for="`${pane.id}` + 'rent_free_months'">JM(月份)</label><input
                  :id="`${pane.id}` + 'rent_free_months'" :name="`${pane.id}` + 'rent_free_months'" :disabled="true"
                  :value="pane.rent_free_months"><br>
              </a-skeleton>
              ...
              <table border="1">
                ...
                <tbody>
                  ...
                  <tr>
                    <td>押金</td>
                    <td>{{ pane.deposit }}</td>
                    <td>{{ dayjs(pane.startdate_and_enddate ? pane.startdate_and_enddate[0] : '').add(3 as number,
                      'day').format('YYYY-MM-DD') }}</td>
                  </tr>
                </tbody>
              </table>
              <slot name="pay">
              ...
              <table border="1" v-if="pane?.payment_detail_items">
                ...
                <tbody>
                  <tr v-for="(payment, index) in planStore.sortedPaymentItems(pane.payment_detail_items)" :key="index">
                    <td style="width: 4%; text-align: center;">{{ index + 1 }}</td>
                    <td style="width: 15%; text-align: center;">{{ dayjs(payment?.period_start).format('YYYY-MM-DD') }}
                    </td>
                    <td style="width: 15%; text-align: center;">{{ dayjs(payment?.period_end).format('YYYY-MM-DD') }}
                    </td>
                    <td style="width: 18%; text-align: right;">{{ planStore?.formatCurrency(payment?.amount) }}</td>
                    <td style="width: 18%; text-align: center;">{{ dayjs(payment?.due_date).format('YYYY-MM-DD') }}</td>
                    <td style="width: 30%; text-align: center;">{{ payment?.remarks }}</td>
                  </tr>
                  <tr>
                    <td colspan="3" style="text-align: center; font-size: 20px;">总金额</td>
                    <td style="text-align: right;">{{ planStore?.formatCurrency(pane?.total_amount as number) }}</td>
                    <td colspan="2"></td>
                  </tr>
                </tbody>
...
import gButtonVue from '@/cp-projectCP/PlanHome/Common/gButton.vue';
import { ref } from 'vue'
